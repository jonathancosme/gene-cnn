...
run_name = 'resnet-smaller-3-2D-12w'
description = 'resnet smaller 3; 2D input; 2D filters; window_size=12; 10% data; 100% labels; species only'
...
if labels_to_use == 'species':
    ...
    def preprocess(*all_cols):
        ...
        inps = text.sliding_window(inps, width=window_size, axis=1)
...
inp_h = processed_val_ds.take(1).get_single_element()[0].shape[1]
inp_w = processed_val_ds.take(1).get_single_element()[0].shape[2]


# In[ ]:


inputs = tf.keras.Input(dtype=tf.float32, shape=(inp_h, inp_w, 1,))
...
with mirrored_strategy.scope():
    ...
    outputs = tf.keras.layers.Conv2D(first_filter_size, (7, 7), padding='same', strides=2, activation=None)(inputs)
    ...
    outputs = tf.keras.layers.MaxPool2D(pool_size=(3,3), padding='same', strides=2)(outputs)
    ...
    for br_i, block_rep in enumerate(res_block_reps):
        ...
        for r_i in np.arange(block_rep):
            ...
            # print(outputs.shape)
            outputs = tf.keras.layers.Conv2D(cur_filter_size, (gen_filter_size, gen_filter_size), strides=1, activation=None)(skip_layer)
            outputs = tf.keras.layers.ZeroPadding2D((pad_size, pad_size))(outputs)
            outputs = tf.keras.layers.BatchNormalization(momentum=bn_momen, scale=SCALE, center=CENTER, epsilon=EPSILON)(outputs)
            outputs = tf.keras.layers.Activation(activation)(outputs)
